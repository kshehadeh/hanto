...
export class File extends Base<BabelFile> {
    ...
    public get imports(): ImportDeclaration[] {
        if (!this._top) return [];
        return this._top.program.body.filter(
            isImportDeclaration,
    ...
    public get exportsDefault(): ExportDefaultDeclaration | undefined {
        ...
        if (isModule(this._top)) {
            return this._top.program.body.find(
    ...
    public get classes(): ClassDeclaration[] {
        ...
        const privatelyDeclaredClasses = this._top.program.body.filter(
            isClassDeclaration,
        ) as ClassDeclaration[];
        const exportedClasses = this.exports.filter(isClassDeclaration);
        const exportedDefaultClass = isClassDeclaration(
            this.exportsDefault?.declaration,
        )
            ? this.exportsDefault.declaration
            : undefined;
        ...
        return [
            ...exportedClasses,
            ...privatelyDeclaredClasses,
            exportedDefaultClass,
