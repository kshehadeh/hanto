...
describe('renderTextAttributesEnd', () => {
    it('should return the ANSI escape code for resetting a color attribute', () => {
        const attributes = { fg: 'blue' };
        const format = 'ansi';
        const expectedOutput = '\x1b[39m';
        expect(_testableFunctions.renderTextAttributesEnd(attributes, format)).toBe(expectedOutput);
    });

    it('should return the ANSIE escape codes for multiple attributes', () => {
        const attributes = { fg: 'blue', bg: 'red', bold: true };
        const format = 'ansi';
...
describe('renderTextAttributesStart', () => {
    it('should return the ANSI escape code for setting a color attribute', () => {
        ...
        expect(_testableFunctions.renderTextAttributesStart(attributes, format)).toBe(expectedOutput);
    });
...
describe('renderSpaceAttributesStart', () => {
    it('should return the ANSI escape code for setting spacing attributes', () => {
        const attributes = { margin: 2 };
        const format = 'ansi';
        const expectedOutput = '\n\n  ';
        expect(_testableFunctions.renderSpaceAttributesStart(attributes, format)).toBe(expectedOutput);
    });

    it('should return the ANSI codes for specific marginXXX attributes over margin when there is contention', () => {
        const attributes = { margin: 2, marginLeft: 1, marginRight: 3, marginTop: 4, marginBottom: 5 };
        const format = 'ansi';
        const expectedOutput = '\n\n\n\n ';
        expect(_testableFunctions.renderSpaceAttributesStart(attributes, format)).toBe(expectedOutput);
    });
...
describe('renderSpaceAttributesEnd', () => {
    it('should return the ANSI escape code for setting spacing attributes', () => {
        ...
        const format = 'ansi';
        const expectedOutput = '  \n\n';
        expect(_testableFunctions.renderSpaceAttributesEnd(attributes, format)).toBe(expectedOutput);
    });
...
describe('buildStart', () => {
    it('should return the ANSI escape code for setting spacing and text attributes in ANSI format', () => {
        const node = { node: 'h1', margin: 2, fg: 'blue' };
        const format = 'ansi';
        const expectedOutput = '\n\n  \x1b[34m';
        expect(_testableFunctions.buildStart(node, format)).toBe(expectedOutput);
    });
    ...
    it('should return an empty string for unsupported format', () => {
        ...
        expect(_testableFunctions.buildStart(node, format as CompilerFormat)).toBe(expectedOutput);
    });
});
